...
let viz_instance =
  (Js.Unsafe.js_expr "Viz")##instance()

let with_viz f =
  Js.Unsafe.meth_call viz_instance "then"
    [| Js.Unsafe.inject (Js.Unsafe.callback f) |] |> ignore

let memgraph_of_bindings (bindings: (string * Obj.t) list) =
  ...
  let conf =
    Memgraph.Dot.config
      ...
      ()
  in
  Memgraph.Dot.print_list ~conf fmt reprs;
  Format.pp_print_flush fmt ();
  Buffer.contents b

let toplevel_idents () =
  let cur_env = !Toploop.toplevel_env in
  let summary = Env.summary cur_env in
  let idents = ref [] in
  let rec loop (s: Env.summary) =
    match s with
    | Env.Env_empty -> ()
    | Env.Env_value (s', id, _) ->
      let name = Ident.name id in
      if not (String.starts_with ~prefix:"_" name) then
        idents := id :: !idents;
      loop s'
    | Env.Env_type (s', _, _) -> loop s'
    | Env.Env_extension (s', _, _) -> loop s'
    | Env.Env_module (s', _, _, _) -> loop s'
    | Env.Env_modtype (s', _, _) -> loop s'
    | Env.Env_class (s', _, _) -> loop s'
    | Env.Env_cltype (s', _, _) -> loop s'
    | Env.Env_open (s', _) -> loop s'
    | Env.Env_functor_arg (s', _) -> loop s'
    | Env.Env_constraints (s', _) -> loop s'
    | Env.Env_copy_types s' -> loop s'
    | Env.Env_persistent (s', _) -> loop s'
    | Env.Env_value_unbound (s', _, _) -> loop s'
    | Env.Env_module_unbound (s', _, _) -> loop s'
  in
  loop summary;
  List.rev !idents
...
let run _ =
  ...
  let rec update_tbl = function
    | [] -> ()
    | id :: ids ->
      match Ident.find_same id !bindings_tbl with
      | _ -> ()
      | exception Not_found ->
        (* newly added ident *)
        let v = Toploop.getvalue (Ident.name id) in
        bindings_tbl := Ident.add id v !bindings_tbl;
        update_tbl ids
  in
  let execute () =
    ...
    let graph_str = memgraph_of_bindings (
      List.map (fun id ->
        Ident.name id, Ident.find_same id !bindings_tbl
      ) toplevel_idents
    ) in
    with_viz (fun viz ->
      memgraph_container##.innerHTML := Js.string "";
      Dom.appendChild memgraph_container
        (viz##renderSVGElement(Js.string graph_str))
    );
    resize ~container ~textbox ()
    >>= fun () ->
    container##.scrollTop := container##.scrollHeight;
